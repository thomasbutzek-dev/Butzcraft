...
function rebuildCharacter(nextProfile) {
    ...
    const shape = getBodyShape(nextProfile.gender, nextProfile.bodyType);
    const scaleX = shape.torsoWidth / 0.95;
    const colors = nextProfile.colors;
    const textures = nextProfile.textures;
    ...
    addHair(characterGroup, nextProfile.hairStyle, nextProfile.gender, hair, colors);
    addAccessory(characterGroup, nextProfile.accessory, colors, accent);

    characterGroup.position.y = 0.08;
    characterGroup.scale.setScalar(0.72);
